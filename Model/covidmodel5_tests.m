...
%%% this code produces:
% Test: Figure S5 panels (a) and (b) (panel (c) was created from them)
...
p_infectHvec=linspace(0,1,10);
T_outvecC=T_outvec(13);
T_invecC=ones(1,1);
...
for kTC=1:length(T_invecC)
    ...
    for quarentineC=2 % 2 - panel (b), 3 - panel (a)
        if quarentineC==4
           xcivec=0; 
        end
        for xciidx=1:length(xcivec)
            xci=xcivec(xciidx);
        ...
        for rel=1:relalizations
            ...
            for step=1:steps
                ...
                if (sum(sum(~~pstates(:,6:8)))/N)<(((3*log(N))/N)) && (QQ==0)
                    quarentine=0;
                else
                ...
                if QQ>0
                  if quarentineC==2 %IQ
                      if abs(week-pweek)==1
                         SWS=step;
                         nodeTest=randsample(N,ceil(xci*N));
                         nodeTestT=randi(24*4*7,1,ceil(xci*N))-1;
                      end
                  end
                  if quarentineC==3 %AQ
                      if abs(week-pweek)==1
                          SWS=step;
                          nodeTest=randsample(find(closeHomeQnode),ceil(2*xci*sum(closeHomeQnode)));
                          nodeTestT=randi(24*4*7,1,ceil(2*xci*sum(closeHomeQnode)))-1;
                      end
                  end
                end
                
                for knodeTestT=find((nodeTestT+SWS)==step) 
                    knodeTest=nodeTest(knodeTestT);
                    nodeState=find(pstates(knodeTest,:));
                    if nodeState>1 && nodeState<6
                       nodeQtest(knodeTest)=1; 
                    end                    
                end
                for nodeTestidx=find(nodeQtest)
                    nodeState=find(pstates(nodeTestidx,:));
                    if nodeState>5
                       nodeQtest(nodeTestidx)=0; 
                    end                    
                end
                if sum(nodeQtest)>0
                closeHomeI= unique([closeHomeI,node2HH(find(nodeQtest))]);
                end
                ...
                vecH=zeros(1,N_H);
                vecH(node2HH(unique([vecE_O(vecE2_O);vecE_H(vecE2_H)])))=1;
            %     vecH(node2HH(unique(vecE_O(vecE2_O))))=1;
                Hi(vecH & Hi==0)=step;
                Hf(logical(vecH))=step;

    
                y(step,:)=y(step,:)+sum(~~states);

                disp([step/steps,rel,xciidx,quarentineC]);
            end
...
%         deltaDf(xciidx)=mean(deltaD)/N;
%         Hpeakf(xciidx)=mean(Hpeak)/N;
%         Vpeakf(xciidx)=mean(Vpeak)/N;
...
%% edit plot
for k1=2
% matCOLOR=[[0.8,0.8,1];[0.6,0.6,1];[0,0,0.8];[0,0,0.6]]; %AQ
% matCOLOR=[[0.2,1,1];[0,1,1];[0,0.8,0.8];[0,0.6,0.6]]; %IQ
axes2 = gca(figure(k1));
hold(axes2,'on');
box(axes2,'on');
set(axes2,'FontSize',40,'FontWeight','bold','LineWidth',6,...
    'PlotBoxAspectRatio',[3 4 1]);
plots=get(gca, 'Children');
% legend(plots(4:1), {'\chi=0','\chi=10^{-1}','\chi=0.25','\chi=0.5'},'EdgeColor',[1,1,1],'Location','northeast','FontSize',32)
